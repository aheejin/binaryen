...
struct RemoveJSUncalled : public Pass {
  ...
  void run(Module* module) override {
    ...
    auto jsCalledFuncs = intrinsics.getJSCalledFunctions();
    for (auto name : jsCalledFuncs) {
      roots.emplace_back(ModuleElementKind::Function, name);
    }
    
    // We ignore all exports.
    ...
    module->removeExports([&](Export* curr) {
      if (curr->kind == ExternalKind::Function) {
        if (std::find(jsCalledFuncs.begin(), jsCalledFuncs.end(), *curr->getInternalName()) == jsCalledFuncs.end()) {
          return true; // Not JS-called, remove export
        }
      } else {
        return true; // Remove all non-function exports unconditionally
      }

      if (curr->kind == ExternalKind::Function) {
        return !needed(ModuleElement(ModuleElementKind::Function, *curr->getInternalName()));
      }
      return false;
    });
