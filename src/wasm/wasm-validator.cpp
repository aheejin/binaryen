...
struct ValidationInfo {
  ...
  // TODO remove
  bool shouldLeftBeSubTypeOfRight(Type left,
                                  Type right,
                                  Expression* curr,
                                  const char* text,
                                  Function* func = nullptr) {
    if (isLeftSubTypeOfRight(left, right)) {
      return true;
    }
    fail(text, curr, func);
    return false;
  ...
  bool shouldLeftBeSubTypeOfRightOrUnreachable(Type left,
                                               Type right,
                                               Expression* curr,
                                               const char* text,
                                               Function* func = nullptr) {
    ...
    return shouldLeftBeSubTypeOfRight(left, right, curr, text, func);
  }
};
...
private:
  ...
  bool shouldLeftBeSubTypeOfRight(Type left,
                                  ...
                                  Expression* curr,
                                  const char* text) {
    return info.shouldLeftBeSubTypeOfRight(
      left, right, curr, text, getFunction());
  }
  ...
  bool shouldLeftBeSubTypeOfRightOrUnreachable(Type left,
                                               ...
                                               Expression* curr,
                                               const char* text) {
    return info.shouldLeftBeSubTypeOfRightOrUnreachable(
      left, right, curr, text, getFunction());
  }
...
void FunctionValidator::visitBlock(Block* curr) {
  ...
  shouldBeTrue(
    curr->type != nullref, curr, "block return type should not be nullref");
}
  ...
  if (!curr->body->is<Block>()) {
    if (!curr->type.isConcrete()) {
      ...
      shouldLeftBeSubTypeOfRightOrUnreachable(
        ...
        curr->type,
        curr,
        "loop with value and last element with value must match types");
    }
  }
  shouldBeTrue(
    curr->type != nullref, curr, "loop return type should not be nullref");
}
...
void FunctionValidator::visitIf(If* curr) {
  ...
  if (!curr->ifFalse) {
    ...
    if (curr->ifTrue->type.isConcrete()) {
      shouldLeftBeSubTypeOfRight(curr->ifTrue->type,
                                 curr->type,
                                 curr,
                                 "if type must match concrete ifTrue");
    }
    if (curr->ifFalse->type.isConcrete()) {
      shouldLeftBeSubTypeOfRight(curr->ifFalse->type,
                                 curr->type,
                                 curr,
  ...
  shouldBeTrue(
    curr->type != nullref, curr, "if return type should not be nullref");
}
...
void FunctionValidator::noteBreak(Name name, Type valueType, Expression* curr) {
  ...
  if (!info.hasBeenSet()) {
    ...
    if (info.type == unreachable) {
      ...
      info.type = getLeastCommonSuperType(info.type, valueType);
    }
    if (arity != info.arity) {
...
void FunctionValidator::visitCall(Call* curr) {
  ...
  for (size_t i = 0; i < curr->operands.size(); i++) {
    if (!shouldLeftBeSubTypeOfRightOrUnreachable(
          curr->operands[i]->type,
          target->params[i],
          curr,
...
void FunctionValidator::visitLocalSet(LocalSet* curr) {
  ...
  if (curr->value->type != unreachable) {
    ...
    shouldLeftBeSubTypeOfRight(curr->value->type,
...
void FunctionValidator::visitSelect(Select* curr) {
  ...
  if (curr->type != unreachable) {
    shouldBeTrue(isLeftSubTypeOfRight(curr->ifTrue->type, curr->type),
                 curr,
                 "select's left expression must be subtype of select's type");
    shouldBeTrue(isLeftSubTypeOfRight(curr->ifFalse->type, curr->type),
                 curr,
                 "select's right expression must be subtype of select's type");
  }
  shouldBeTrue(
    curr->type != nullref, curr, "select return type should not be nullref");
}
...
void FunctionValidator::visitRefIsNull(RefIsNull* curr) {
  shouldBeTrue(curr->anyref->type == unreachable || curr->anyref->type.isRef(),
               curr->anyref,
...
void FunctionValidator::visitTry(Try* curr) {
  ...
  shouldBeTrue(
    curr->type != nullref, curr, "try return type should not be nullref");
}
...
void FunctionValidator::visitFunction(Function* curr) {
  ...
  shouldBeTrue(curr->result != nullref,
               curr,
               "function return type should not be nullref");
...
static void validateBinaryenIR(Module& wasm, ValidationInfo& info) {
  struct BinaryenIRValidator
    ...
    void visitExpression(Expression* curr) {
      ...
      if (newType != oldType && !isLeftSubTypeOfRight(newType, oldType)) {
...
static void validateGlobals(Module& module, ValidationInfo& info) {
  ModuleUtils::iterDefinedGlobals(module, [&](Global* curr) {
    ...
    assert(curr->init);
    info.shouldBeTrue(
      curr->init->isConstExpression(), curr->name, "global init must be valid");

    if (!info.shouldLeftBeSubTypeOfRight(
          curr->init->type,
          curr->type,
          curr->init,
